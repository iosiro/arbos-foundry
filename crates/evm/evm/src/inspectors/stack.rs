...
use foundry_evm_core::{
    ContextExt, Env, Evm, FoundryContext, InspectorExt,
...
impl InspectorStackRefMut<'_> {
    ...
    fn adjust_evm_data_for_inner_context(
        &mut self,
        ecx: &mut FoundryContext<&mut dyn DatabaseExt>,
    ) {
    ...
    fn top_level_frame_start(&mut self, ecx: &mut FoundryContext<&mut dyn DatabaseExt>) {
        if self.enable_isolation {
...
impl Inspector<FoundryContext<&mut dyn DatabaseExt>> for InspectorStackRefMut<'_> {
    ...
    fn selfdestruct(&mut self, contract: Address, target: Address, value: U256) {
        call_inspectors!([&mut self.printer], |inspector| Inspector::<
            FoundryContext<&mut dyn DatabaseExt>,
        >::selfdestruct(
...
impl Inspector<FoundryContext<&mut dyn DatabaseExt>> for InspectorStack {
    ...
    fn call(
        &mut self,
        context: &mut FoundryContext<&mut dyn DatabaseExt>,
