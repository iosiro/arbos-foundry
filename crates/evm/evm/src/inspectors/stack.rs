...
use foundry_evm_core::{
    ...
    evm::{FoundryEvmContext, new_evm_with_inspector},
};
...
impl InspectorStackRefMut<'_> {
    ...
    fn adjust_evm_data_for_inner_context(&mut self, ecx: &mut FoundryEvmContext<&mut dyn DatabaseExt>) {
    ...
    fn do_call_end(
        &mut self,
        ecx: &mut FoundryEvmContext<&mut dyn DatabaseExt>,
        inputs: &CallInputs,
    ...
    fn top_level_frame_start(&mut self, ecx: &mut FoundryEvmContext<&mut dyn DatabaseExt>) {
        if self.enable_isolation {
...
impl Inspector<FoundryEvmContext<&mut dyn DatabaseExt>> for InspectorStackRefMut<'_> {
...
impl Inspector<FoundryEvmContext<&mut dyn DatabaseExt>> for InspectorStack {
    fn step(
        &mut self,
        interpreter: &mut Interpreter,
        ecx: &mut FoundryEvmContext<&mut dyn DatabaseExt>,
    ...
    fn call(
        &mut self,
        context: &mut FoundryEvmContext<&mut dyn DatabaseExt>,
