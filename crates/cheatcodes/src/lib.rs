...
use foundry_evm_core::{FoundryContext, backend::DatabaseExt};
use spec::Status;
...
pub struct CheatsCtxt<'cheats, 'evm, 'db, 'db2> {
    ...
    /// The EVM data.
    pub(crate) ecx: &'evm mut FoundryContext<&'db mut (dyn DatabaseExt + 'db2)>,
...
impl<'db, 'db2> std::ops::Deref for CheatsCtxt<'_, '_, 'db, 'db2> {
    type Target = FoundryContext<&'db mut (dyn DatabaseExt + 'db2)>;
